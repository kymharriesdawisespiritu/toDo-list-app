...
function showingData() {
  ...
  tasksArray.forEach((task) => {
    ...
    <div class="task" task-id="${task.id}">
      <label for="${task.id}">
        <input type="checkbox" onclick="updateStatue(this)" class="check" id="${
          task.id
        }" ${task.pend == "no" ? "checked" : ""}>
...
function updateStatue(task) {
  for (let i = 0; i < tasksArray.length; i++) {
    if (
      tasksArray[i].id ==
      task.parentElement.parentElement.getAttribute("task-id")
    ) {
      tasksArray[i].pend == "yes"
        ? (tasksArray[i].pend = "no")
        : (tasksArray[i].pend = "yes");
    }
    localStorage.setItem("tasks", JSON.stringify(tasksArray));
  }
